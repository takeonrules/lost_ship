...
module LostShip
  module Schemas
    ...
    FormationSlot = Dry::Schema.JSON do
    ...
    Fleet = Dry::Schema.JSON do
      optional(:formation).array(FormationSlot)
      required(:ship).hash(Ship)
      required(:scouts).array(Scout)
      required(:pilots).array(Pilot)
    end
    ...
    Game = Dry::Schema.JSON do
      optional(:threat).hash(Threat)
      required(:fleet).hash(Fleet)
    end
  end
end
